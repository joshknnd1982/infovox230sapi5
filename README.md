...
**Install only what you want.** The installer's components page lists the twelve
languages and, inside each, its five voices, and every one of them can be ticked
or left out on its own. A language is what costs disk space — it carries its own
pronunciation rules, 37 KB for Spanish to 6.7 MB for Danish — and an unticked
language is never written. A voice costs only a line in the Windows voice list.
The choice is recorded in `installed.ini` and read back by
[`src/ivx_catalog.cpp`](src/ivx_catalog.cpp), which publishes those voices and
no others; the components, the files and that record are all generated from the
engine's own mode table by
...
| [`installer/voices.iss`](installer/voices.iss) | its languages and voices, generated from the mode table |
| [`tools/gen_installer_voices.py`](tools/gen_installer_voices.py) | what generates them |
...
Two files in the tree are generated from `bin/infovox230/modes.json`, the mode
table taken from a working Infovox 230 installation. Neither is regenerated by
the build; run them by hand after changing that table:
...
- The installer's components page now lists the twelve languages and the five
  voices inside each, all of them optional. Setup types for everything, the
  English voices only, or American English Male and Female; and re-running setup
  starts from your previous choice and removes what you clear.
- Only the languages you choose are written to disk, which is most of the
  installed size: 23 MB of pronunciation rules if you take all twelve, 2.4 MB if
  you take American English alone.
- The catalogue publishes only the voices that were installed, and refuses to
  publish one whose rule file is not on the disk — a voice in the Windows list
  that cannot speak is worse than one that was never offered.
- Naming a built-in voice in `voices.ini` asks for it, so a single voice left
  out at install time can be put back without running the installer again.
